...
export default function VendorStorefront() {
  const { slug } = useParams(); // this is now the username/store slug (e.g. "tenibags")
  ...
  useEffect(() => {
    ...
    const fetchVendor = async () => {
      try {
        // New endpoint: /api/stores/@slug
        const vendorRes = await api.get(`/stores/@${slug}`);
        setVendor(vendorRes.data);

        const productsRes = await api.get(`/stores/@${slug}/products`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl text-gray-600">Loading @{slug}...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-4">@{slug} not found</h1>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-end gap-6">
            ...
            <div className="text-white">
              <h1 className="text-3xl md:text-5xl font-bold">@{vendor.slug}</h1>
              <h2 className="text-xl">{vendor.shop_name}</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="md:col-span-2">
            <h2 className="text-2xl font-bold mb-4">About @{vendor.slug}</h2>
            <p className="text-gray-700 leading-relaxed">{vendor.description || 'No description yet.'}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6 h-fit">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <a href={`mailto:hello@${vendor.slug}.com`} className="text-orange-600 hover:underline">
                  hello@{vendor.slug}.com
                </a>
              </div>
              <div className="flex items-center gap-3">
